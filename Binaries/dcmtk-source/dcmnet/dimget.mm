        ...
        DcmDataset **rspIds)
{
    ...
    DcmQueryRetrieveSCP storage(storageConfig, storageOptions, factory);
    storage.setSecureConnection(assoc->params->DULparams.useSecureLayer);
    @try
    {
    int index = 0;
    ...
    while (cond == EC_Normal && status == STATUS_Pending) {
        ...
        if (!selectReadable(assoc, blockMode, timeout))
        {
		...
		switch (rsp.CommandField)
		{
			...
			case DIMSE_C_STORE_RQ:
				 cond = storage.storeSCP(assoc, &rsp.msg.CStoreRQ, presID, *dbHandle, OFFalse, OFTrue);
