...
final class Probe: NSManagedObject {
    var publicImageCount: NSNumber = 12
    @objc var noFiles: NSNumber {
        precondition(Thread.isMainThread, "image count read off its owning context")
        return publicImageCount
    }
...
// Return literal attributes; N2XMLRPC alone owns escaping at the wire boundary.
object.setPrimitiveValue("a <b> & c", forKey: "name")
let literal = XMLRPCOwnedThreadRead.dictionary(for: object)
precondition(literal["name"] == "a <b> & c", "literal: \(literal)")

// Empty, single-frame, multiframe, mixed study and an invalidated zero cache.
// Ask the public getter; neither abs(cache) nor a write to storage is correct
// for every case. The real Study/Series getters are exercised in native runs.
for (cached, count) in [(0, 0), (4, 4), (-4, 4), (-8, 8), (0, 8)] {
    object.setPrimitiveValue(cached, forKey: "numberOfImages")
    object.publicImageCount = NSNumber(value: count)
    let exported = XMLRPCOwnedThreadRead.dictionary(for: object)
    precondition(exported["numberOfImages"] == String(count), "cached sentinel leaked: \(exported)")
    precondition((object.primitiveValue(forKey: "numberOfImages") as! NSNumber).intValue == cached,
                 "serialization rewrote the internal cache")
}
...
Thread.detachNewThread {
    ...
    let value = XMLRPCOwnedThreadRead.onMainAndWait {
        ...
        precondition(XMLRPCOwnedThreadRead.dictionary(for: object)["numberOfImages"] == "8")
        return "from-main"
    }
