...
def descriptor_snapshot(pid):
    """Count numeric descriptors and TCP state for this process only."""
    done = subprocess.run(['lsof', '-a', '-p', str(pid), '-nP', '-F', 'pftT'],
                          capture_output=True, text=True, timeout=5)
    if done.returncode != 0:
        raise RuntimeError('cannot inspect helper descriptors: ' + done.stderr.strip())
    descriptors = set()
    closed = set()
    descriptor = None
    for field in done.stdout.splitlines():
        if field.startswith('p'):
            if field != 'p' + str(pid):
                raise RuntimeError('lsof returned another process: ' + field)
        elif field.startswith('f'):
            descriptor = int(field[1:]) if field[1:].isdigit() else None
            if descriptor is not None:
                descriptors.add(descriptor)
        elif field == 'TST=CLOSED' and descriptor is not None:
            closed.add(descriptor)
    return len(closed), len(descriptors)
...
with tempfile.TemporaryDirectory(prefix='horos-asyncsocket-reset-') as tmp:
    ...
    try:
        ...
        # Warm up lazy CFNetwork resources before measuring the reset batch.
        before_closed, before_fds = descriptor_snapshot(server.pid)
        ...
        samples = []
        while True:
            after_closed, after_fds = descriptor_snapshot(server.pid)
...
if failures:
    raise SystemExit(1)
print('PASS: reset-during-open frees owned descriptors; sanitizers silent')
