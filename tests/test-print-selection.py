...
@main struct Check {
    ...
    static func main() {
        ...
        single.frameID = 3
        let overlap = PrintSelection.job(from: [single, series], source: "effective")
        precondition(overlap.entries.map(\.frame) == [3, 0, 1, 2, 4, 5, 6, 7])
        precondition(overlap.entries.count == 8, "overlapping selections must not duplicate frame 3")
