...
for xib in (root / 'Horos/Resources/en.lproj/Viewer.xib',
    ...
    missing = [tag for tag in sliders
               if 'customClass="HorosCellSlider"' not in tag
               and 'customClass="HorosImageNavigationSlider"' not in tag]
...
struct Probe {
    static func main() {
        ...
        var lastHosts = 0
        for cycle in 1...12 {
            // The window must actually die each cycle, or the footprint measures
            // retained backing stores instead of AttributeGraph tables. Drawing
            // anything into a retained window looks like a leak otherwise.
            ...
            settle(0.25)
            precondition(weakWindow == nil, "cycle \(cycle): probe window survived close")
        }
        settle(0.8)
        let end = residentMB()
