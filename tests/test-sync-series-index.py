...
@main struct Check {
    static func main() {
        ...
        // direction, wrapping around either end.
        ...
        for (difference, normal, flipped) in [(3, 8, 2), (100, 5, 5), (-100, 5, 5),
            ...
            precondition(S.relativeIndex(current: 5, difference: difference,
                                          count: 10, flippedData: false) == normal)
            precondition(S.relativeIndex(current: 5, difference: difference,
                                          count: 10, flippedData: true) == flipped)
        }
        // Overflow controls: no abs(Int.min), negation of Int.min, or unchecked sum.
        precondition(S.relativeIndex(current: 5, difference: Int.min, count: 10, flippedData: false) == 7)
        precondition(S.relativeIndex(current: 5, difference: Int.min, count: 10, flippedData: true) == 3)
        precondition(S.relativeIndex(current: Int.max - 1, difference: Int.max - 1,
                                      count: Int.max, flippedData: false) == Int.max - 2)
        precondition(S.relativeIndex(current: Int.max - 1, difference: Int.min,
                                      count: Int.max, flippedData: true) == 0)
        for difference in [Int.min, Int.max] {
            for flipped in [false, true] {
                precondition(S.relativeIndex(current: 0, difference: difference,
                                              count: 1, flippedData: flipped) == 0)
            }
        }
        // The legacy size predicate remains available, without gating valid moves.
        ...
        precondition(!S.relativeMoveFitsInOneWrap(difference: Int.min, count: 10))
        // Compare against repeated single-step movement, independently of modulo.
        for count in [1, 2, 7, 16] {
            for current in 0..<count {
                for difference in [-103, -100, -27, -16, -3, 0, 3, 16, 27, 100, 103] {
                    for flipped in [false, true] {
                        var expected = current
                        let step = (difference < 0 ? -1 : 1) * (flipped ? -1 : 1)
                        for _ in 0..<abs(difference) {
                            expected += step
                            if expected == count { expected = 0 }
                            if expected == -1 { expected = count - 1 }
                        }
                        let index = S.relativeIndex(current: current, difference: difference,
                                                    count: count, flippedData: flipped)
                        precondition(index == expected && index >= 0 && index < count)
                    }
                }
            }
        }
