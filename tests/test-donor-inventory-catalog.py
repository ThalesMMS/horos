...
The catalog must name every donor novelty, the 17 forwarded scopes, plugin
and DICOMweb contracts, and the existing host regressions. It must not copy
those suites, import donor sources, or treat an issue number as proof.
"""
...
CATALOG_SOURCE_PATH = "donor checkout (HOROS_DONOR_CHECKOUT)"


def donor_checkout():
    """The read-only donor reference, or None when this machine has no copy.

    No clone provides it. Point HOROS_DONOR_CHECKOUT at it to run the checks
    that compare against the origin tree; without it the catalogued SHA and
    every workbench-side assertion are still verified.
    """
    value = os.environ.get("HOROS_DONOR_CHECKOUT")
...
def test_catalog_files_exist_and_name_the_issue():
    ...
    assert catalog["source"]["path"] == CATALOG_SOURCE_PATH
...
def source_tree_paths():
    """Paths under Horos/ at the catalogued revision, or None when absent.

    The checkout is a read-only reference whose working tree may sit on any
    revision: later phases read other commits from the same clone. The catalog
    describes one revision, so read that revision's tree instead of the files
    that happen to be checked out.
    """
    source = donor_checkout()
    if source is None or not (source / ".git").exists():
        return None
    if subprocess.run(["git", "-C", str(source), "cat-file", "-e", SOURCE_SHA + "^{commit}"],
                      capture_output=True).returncode != 0:
        return None
    listing = subprocess.check_output(
        ["git", "-C", str(source), "ls-tree", "-r", "--name-only", SOURCE_SHA, "Horos/"], text=True)
    return set(listing.splitlines())


def test_source_sha_is_present_in_local_donor_when_present():
    source = donor_checkout()
    if source is None or (not (source / ".git").exists() and not (source / "Horos").exists()):
        print("note: donor checkout absent; catalog SHA is still pinned")
        return
    if not (source / ".git").exists():
        print("note: donor checkout is not a git checkout; catalog SHA is still pinned")
        return
    present = subprocess.run(["git", "-C", str(source), "cat-file", "-e", SOURCE_SHA + "^{commit}"],
                             capture_output=True)
    assert present.returncode == 0, f"the donor checkout does not contain the catalogued SHA {SOURCE_SHA}"
...
def test_novelties_classify_donor_only_sources_without_copying():
    catalog = load_catalog()
    classified = set()
    for item in catalog["novelties"] + catalog["exclusions"]:
        ...
        for path in item["files"]:
            ...
            if path.startswith("Horos/"):
                ...
                # paths in the donor's Horos/Sources, so they are not classified
                # against that tree, but they do have to exist here.
                assert existing(path), path
    tree = source_tree_paths()
    if tree is None:
        assert classified, "catalog lists no donor files"
        return
    workbench = {
    ...
    for original, replacement in aliases.items():
        assert original in tree, original
    ...
    only = set()
    for tracked in tree:
        if not tracked.startswith("Horos/Sources/") or Path(tracked).suffix not in {".swift", ".m", ".mm", ".h", ".cpp", ".metal"}:
            continue
        workbench_relative = tracked[len("Horos/"):]
        if workbench_relative not in workbench and tracked not in aliases:
            only.add(tracked)
    missing = sorted(only - classified)
    extra = sorted(classified - only - {"Horos/Sources/MetalViewer/MetalShaders.metal"})
    assert not missing, "donor-only sources missing from catalog:\n" + "\n".join(missing)
    # Extra classified paths are allowed only when they exist in the donor tree.
    for path in extra:
        assert path in tree or path.endswith("MetalShaders.metal"), path
...
test_source_sha_is_present_in_local_donor_when_present()
test_seventeen_forwarded_scopes_have_unique_ids_and_owners()
test_every_criterion_has_one_owner_and_stable_id()
test_novelties_classify_donor_only_sources_without_copying()
...
print("PASS: donor inventory catalog covers novelties, 17 forwarded scopes, plugins, DICOMweb and reuse")
