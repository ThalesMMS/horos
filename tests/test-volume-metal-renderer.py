...
def sparse_phantom(i, j, k):
    return 400.0 if 12 <= i <= 35 and 7 <= j <= 25 and k in (7, 8, 16, 24) else -1024.0
...
func sparsePhantom(_ i: Int, _ j: Int, _ k: Int) -> Float { (12...35).contains(i) && (7...25).contains(j) && [7,8,16,24].contains(k) ? 400 : -1024 }
...
@main struct Check {
    static func main() throws {
        ...
        let sparse = try ResliceVolume(width: 48, height: 32, depth: 40, voxels: voxels(48, 32, 40, sparsePhantom), voxelToWorld: matrix_identity_float4x4)
        try engine.upload(sparse)
        ...
        try engine.upload(iso)
        let regionTransfer = try VolumeTransferFunction(level: 120, width: 240, colour: clutData(twoTone), opacity: VolumeTransferFunction.opacityTable(points: []))
        for parallel in [true, false] {
            ...
            for mode in [VolumeRenderingMode.maximum, .composite] {
                ...
                let part = try engine.render(request(SIMD2(11, 9), origin: SIMD2(7, 5)))
                var expectedColour = Data(), expectedScalar = Data()
                for row in 5..<14 {
                    let start = (row * 31 + 7) * 4
                    expectedColour.append(full.bgra.subdata(in: start..<(start + 11 * 4)))
                    expectedScalar.append(full.scalar.subdata(in: start..<(start + 11 * 4)))
                }
                precondition(part.bgra == expectedColour && part.scalar == expectedScalar, "subregion changed the camera rays")
                precondition(Array(full.bgra) == saved, "pooled render overwrote an earlier result")
                messages["region-\(parallel)-\(mode.rawValue)"] = "equal"
...
def main():
    ...
    volumes = {'phantom': phantom, 'slab': slab_phantom, 'sparse': sparse_phantom,
    ...
    for parallel in ('true', 'false'):
        for mode in (0, 1):
            assert m['region-%s-%d' % (parallel, mode)] == 'equal'
    assert 'outside the viewport' in m['region-outside']
