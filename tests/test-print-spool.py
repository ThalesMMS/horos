...
@main struct Check {
    static func main() {
        ...
        let report = PrintRaster()
        report.pdfBytes = pdf!
        let wrapped = PrintSelection.pagePDF(from: report, orientation: "asStored")
        precondition(wrapped == report.pdfBytes)
        let corrupt = PrintRaster()
        corrupt.pdfBytes = Data("%PDF-1.4 report".utf8)
        precondition(PrintSelection.pagePDF(from: corrupt, orientation: "asStored") == nil)
        ...
        precondition(cancelled.pages.isEmpty, "cancelled jobs must expose no printable prefix")
