...
def test_source_sha_is_present_in_local_ystarrev_when_present():
    source = (root / "../ystarrev/horos").resolve()
    if not (source / ".git").exists() and not (source / "Horos").exists():
        print("note: ../ystarrev/horos absent; catalog SHA is still pinned")
        return
    if not (source / ".git").exists():
        print("note: ../ystarrev/horos is not a git checkout; catalog SHA is still pinned")
        return
    present = subprocess.run(["git", "-C", str(source), "cat-file", "-e", SOURCE_SHA + "^{commit}"],
                             capture_output=True)
    assert present.returncode == 0, f"../ystarrev/horos does not contain the catalogued SHA {SOURCE_SHA}"
...
test_source_sha_is_present_in_local_ystarrev_when_present()
