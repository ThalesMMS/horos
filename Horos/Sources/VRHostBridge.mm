...
- (NSDictionary *)horosVolumeSnapshot {
    ...
    float cosines[9];
    [first orientation:cosines];
    NSArray *transform = @[@(cosines[0] * sx), @(cosines[3] * sy), @(cosines[6] * dz), @(first.originX),
                           @(cosines[1] * sx), @(cosines[4] * sy), @(cosines[7] * dz), @(first.originY),
                           @(cosines[2] * sx), @(cosines[5] * sy), @(cosines[8] * dz), @(first.originZ),
                           @0, @0, @0, @1];
...
- (NSData *)horosVolumeMetalRenderWithCamera:(NSArray<NSNumber *> *)camera near:(double)near far:(double)far
    ...
    if (!reason) {
        ...
        bytes = [renderer renderWithCamera:snapshot[@"camera"] near:[snapshot[@"near"] doubleValue] far:[snapshot[@"far"] doubleValue]
                          ...
                          scalarBackground:[snapshot[@"scalarBackground"] doubleValue] scalarOut:scalarOut error:&failure];
