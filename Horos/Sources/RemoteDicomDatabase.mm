...
static NSData *HorosSendDatabaseRequest(NSData *request, NSString *address, NSInteger port,
                                        NSInteger (^handler)(NSData *data))
{
    NSThread *thread = NSThread.currentThread;
    NSError *error = nil;
    NSData *response = [HorosDatabaseTransport sendRequest: request
                                                   toHost: address ?: @""
                                                     port: port
                                                receiving: handler ? ^NSInteger(NSData *data, NSError **handlerError) {
                                                    // An Objective-C exception must not cross into Swift.
                                                    @try { return handler(data); }
                                                    @catch (NSException *exception) {
                                                        if (handlerError)
                                                            *handlerError = [NSError errorWithDomain: @"HorosDatabaseResponse" code: 1
                                                                userInfo: @{NSLocalizedDescriptionKey: exception.reason ?: exception.name}];
                                                        return -1;
                                                    }
                                                } : (NSInteger (^)(NSData *, NSError **))nil
                                                cancelled: ^BOOL { return thread.isCancelled; }
                                                    error: &error];
    if (!response)
        [NSException raise: NSObjectInaccessibleException format: @"%@",
            error.localizedDescription ?: NSLocalizedString(@"The shared-database request failed.", nil)];
    return response;
}
...
-(NSData*)synchronousRequest:(NSData*)request urgent:(BOOL)urgent dataHandlerTarget:(id)target selector:(SEL)sel context:(void*)context {
    ...
    BOOL acquired = NO;
    if (!urgent && _connectionsSemaphoreId) {
        while (!acquired) {
            if (NSThread.currentThread.isCancelled)
                [NSException raise:NSGenericException format:@"%@", NSLocalizedString(@"The shared-database request was cancelled.", nil)];
            acquired = dispatch_semaphore_wait(_connectionsSemaphoreId, dispatch_time(DISPATCH_TIME_NOW, 100 * NSEC_PER_MSEC)) == 0;
        }
    }
    
    NSInteger (^handler)(NSData *) = nil;
    if (target && sel) {
        __unsafe_unretained id unretainedTarget = target;
        handler = ^NSInteger(NSData *data) {
            NSInteger (*implementation)(id, SEL, id, id, void*) = (NSInteger (*)(id, SEL, id, id, void*))
                [unretainedTarget methodForSelector: sel];
            return implementation(unretainedTarget, sel, nil, data, context);
        };
    }
    ...
    @try {
        ...
        NSInteger attempts = [HorosSharedDatabaseCommand isRetryableRequest: request] ? 3 : 1;
        NSException *lastFailure = nil;
        for (NSInteger attempt = 0; attempt < attempts; ++attempt) {
            if (NSThread.currentThread.isCancelled) break;
            @try {
                if (attempt > 0) {
                    // Never append a second response to a partial first one.
                    if (sel == @selector(_connection:handleData_fetchDatabaseIndex:context:)) {
                    ...
                    [NSThread sleepForTimeInterval: 0.1 * attempt * attempt];
                }
                return HorosSendDatabaseRequest(request, self.address, self.port, handler);
            } @catch (NSException* e) {
                lastFailure = e;
                N2LogExceptionWithStackTrace(e);
            }
        ...
        if (lastFailure) {
            NSString *guidance = [HorosSharedDatabaseCommand actionRequiredForRequest: request];
            if (guidance.length)
                NSLog(@"---- shared database: %@ %@", lastFailure.reason ?: lastFailure.name, guidance);
        }
    } @finally {
        if (acquired)
