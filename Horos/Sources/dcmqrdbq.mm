...
static const DB_FindAttr TbFindAttr [] = {
        DB_FindAttr( DCM_PatientsBirthDate ,                    PATIENT_LEVEL,  OPTIONAL_KEY ),
        DB_FindAttr( DCM_PatientsSex,                           PATIENT_LEVEL,  OPTIONAL_KEY ),
        DB_FindAttr( DCM_PatientsName,                          PATIENT_LEVEL,  REQUIRED_KEY ),
        DB_FindAttr( DCM_PatientID,                             PATIENT_LEVEL,  UNIQUE_KEY ),
        DB_FindAttr( DCM_PatientsBirthTime,                     PATIENT_LEVEL,  OPTIONAL_KEY ),
     ...
     //   DB_FindAttr( DCM_NumberOfPatientRelatedInstances,       PATIENT_LEVEL,  OPTIONAL_KEY ),
        ...
        DB_FindAttr( DCM_ReferringPhysiciansName,               STUDY_LEVEL,    OPTIONAL_KEY ),
   ...
   //     DB_FindAttr( DCM_OtherStudyNumbers,                     STUDY_LEVEL,    OPTIONAL_KEY ),
   //     DB_FindAttr( DCM_AdmittingDiagnosesDescription,         STUDY_LEVEL,    OPTIONAL_KEY ),
   //     DB_FindAttr( DCM_PatientsAge,                           STUDY_LEVEL,    OPTIONAL_KEY ),
   //     DB_FindAttr( DCM_PatientsSize,                          STUDY_LEVEL,    OPTIONAL_KEY ),
  //      DB_FindAttr( DCM_PatientsWeight,                        STUDY_LEVEL,    OPTIONAL_KEY ),
...
//	NSLog(@"next Move response: %d", *numberOfRemainingSubOperations);
    ...
    if (cond.good() && !HorosFindSOPClassAndInstanceInDataSet(fileformat.getDataset(),
            SOPClassUID, DICOM_UI_LENGTH + 1, SOPInstanceUID, DICOM_UI_LENGTH + 1))
...
{	
    ...
    if (m == NULL) m = "XX";
    snprintf(prefix, sizeof(prefix), "%.16s_%d_", m, getpid());
    unsigned seedvalue = seed.fetch_add(1, std::memory_order_relaxed) + 1;
    ...
    if (filename.length() > MAXPATHLEN) return DcmQROsiriXDatabaseError;
    memcpy(newImageFileName, filename.c_str(), filename.length() + 1);
...
// Public DCMTK interface: validate capacities before consuming a DB result.
OFCondition DcmQueryRetrieveOsiriXDatabaseHandle::makeNewStoreFileName(
    const char* sopClass, const char* sopInstance, char* path, size_t capacity)
{
    if (!path || capacity < MAXPATHLEN + 1) return EC_IllegalParameter;
    return makeNewStoreFileName(sopClass, sopInstance, path);
}

OFCondition DcmQueryRetrieveOsiriXDatabaseHandle::nextFindResponse(
    DcmDataset** identifiers, DcmQueryRetrieveDatabaseStatus* status,
    const DcmQueryRetrieveCharacterSetOptions&)
{
    // OsiriXSCPDataHandler owns response encoding and SpecificCharacterSet.
    return nextFindResponse(identifiers, status);
}

OFCondition DcmQueryRetrieveOsiriXDatabaseHandle::nextMoveResponse(
    char* sopClass, size_t classCapacity, char* sopInstance, size_t instanceCapacity,
    char* path, size_t pathCapacity, unsigned short* remaining,
    DcmQueryRetrieveDatabaseStatus* status)
{
    if (!sopClass || !sopInstance || !path || classCapacity < DICOM_UI_LENGTH + 1 ||
        instanceCapacity < DICOM_UI_LENGTH + 1 || pathCapacity < MAXPATHLEN + 1)
        return EC_IllegalParameter;
    return nextMoveResponse(sopClass, sopInstance, path, remaining, status);
}
