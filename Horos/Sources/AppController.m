...
- (void) runPreferencesUpdateCheck:(NSTimer*) timer
{
    ...
    @try {
        BOOL seriesListModeChanged = [[previousDefaults valueForKey: @"UseFloatingThumbnailsList"] boolValue] != [defaults boolForKey: @"UseFloatingThumbnailsList"];
        NSWindow *seriesListKeyWindow = seriesListModeChanged ? NSApp.keyWindow : nil;
        ...
        if( seriesListModeChanged || [[previousDefaults valueForKey: @"SeriesListVisible"] intValue] != [defaults integerForKey: @"SeriesListVisible"])
        {
            ...
            if( seriesListKeyWindow.isVisible)
...
- (IBAction) killAllStoreSCU:(id) sender
{
	...
	BOOL persistListenerMode = ![[NSUserDefaults standardUserDefaults] hasArgumentOverrideForKey:@"hideListenerError"];
...
//				[[NSUserDefaults standardUserDefaults] setBool:NO forKey:@"DONTCOPYWLWWSETTINGS"];
				...
				if( ![[NSUserDefaults standardUserDefaults] hasArgumentOverrideForKey:@"hideListenerError"] &&
                    [[NSUserDefaults standardUserDefaults] objectForKey: @"copyHideListenerError"])
...
//			[NSTimer scheduledTimerWithTimeInterval:2.0 target:splashController selector:@selector(windowShouldClose:) userInfo:nil repeats:0]; 
    ...
    //if ([NSUserDefaults.standardUserDefaults boolForKey:@"DoNotEmptyIncomingDir"]) // move temp & decompress to incoming
    {
        ...
        for (NSString* path in [NSArray arrayWithObjects: temporary, [[DicomDatabase activeLocalDatabase] decompressionDirPath], nil])
            for (NSString* f in [[NSFileManager defaultManager] enumeratorAtPath:path filesOnly:NO recursive:NO])
            {
                ...
                if ([path isEqualToString:temporary] &&
                    [HorosImageExportPath isPrivateExportDirectoryAtPath:[path stringByAppendingPathComponent:f]])
                    continue;
...
- (IBAction)printFromMenu:(id)sender
{
    ...
    if( [controller isKindOfClass:[BrowserController class]])
        [(BrowserController *)controller printDatabaseSelection:sender];
    else
        [NSApp sendAction:@selector(print:) to:nil from:sender];
}

- (BOOL)validatePrintFromMenu:(NSMenuItem *)item
{
    if( [[[NSApp keyWindow] windowController] isKindOfClass:[BrowserController class]])
        return YES;

    // Preserve the existing responder and its validation in other windows.
    id target = [NSApp targetForAction:@selector(print:) to:nil from:item];
    if( target == nil)
        return NO;
    NSMenuItem *legacyItem = [item copy];
    [legacyItem setAction:@selector(print:)];
    if( [target respondsToSelector:@selector(validateMenuItem:)])
        return [target validateMenuItem:legacyItem];
    if( [target respondsToSelector:@selector(validateUserInterfaceItem:)])
        return [target validateUserInterfaceItem:legacyItem];
    return YES;
}
...
{
    if( [item action] == @selector(printFromMenu:))
