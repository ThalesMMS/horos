...
- (id)copyWithZone:(NSZone *)zone
{
    ...
    copy->loadedFileRevision = [self->loadedFileRevision retain];
    ...
    copy->inverseVal = self->inverseVal;
...
#pragma mark *loading a frame
            ...
            BOOL keepNuclearMedicinePolarity = [[dcmObject attributeValueWithName:@"Modality"] isEqualToString:@"PT"] ||
                ([[NSUserDefaults standardUserDefaults] boolForKey:@"OpacityTableNM"] && [[dcmObject attributeValueWithName:@"Modality"] isEqualToString:@"NM"]);
            // MONOCHROME1 controls presentation polarity, not calibrated data.
            // Negating fImage here changed every ROI/plugin measurement.
            inverseVal = [colorspace isEqualToString:@"MONOCHROME1"] && !keepNuclearMedicinePolarity;
...
#ifndef DECOMPRESS_APP
        if( fImage && self.srcFile)
            loadedFileRevision = [[HorosFileRevision alloc] initWithPath: self.srcFile];
#endif
    }
}
...
{
    return atomic_load_explicit( &horosDecodedFrameCount, memory_order_relaxed);
}

+ (void) resetDecodedFrameCount
{
    atomic_store_explicit( &horosDecodedFrameCount, 0, memory_order_relaxed);
}
...
- (id) dicomPreviewWindow
{
    ...
    // savedWW/savedWL hold Window Width/Center as the file gave them, after the
    // sign fix the loader applies. A width of zero is the file saying nothing,
    // not a window one unit wide.
    if( savedWW == 0 || isnan( savedWW) || isnan( savedWL) || isinf( savedWW) || isinf( savedWL))
        return nil;
    
    return [[[HorosPreviewWindow alloc] initWithLevel: savedWL width: savedWW source: HorosPreviewWindowSourceDicom] autorelease];
#else
    return nil;
#endif
}
...
{
#ifndef DECOMPRESS_APP
    // The one decode this frame needs; it is shared with whatever draws it.
    [self CheckLoad];
    
    // A colour frame has no scalar intensity, and a frame with no pixels has
    // nothing to sample. Neither is a failure worth guessing around.
    if( isRGB || fImage == nil || width < 1 || height < 1)
        return nil;
    
    return [HorosPreviewAutomaticWindow windowForValues: fImage width: width height: height modality: self.modalityString];
#else
    return nil;
#endif
}
...
- (id) frameRangePreviewWindow
{
    ...
    // The same minimum and maximum a request of (0,0) has always used here.
    // Reading them costs two passes over the frame already decoded, and is
    // only asked for when the sampled calculation had nothing to work with.
    [self computePixMinPixMax];
    
    if( fullww <= 0 || isnan( fullww) || isinf( fullww) || isnan( fullwl) || isinf( fullwl))
        return nil;
    
    return [[[HorosPreviewWindow alloc] initWithLevel: fullwl width: fullww source: HorosPreviewWindowSourceFrameRange] autorelease];
#else
    return nil;
#endif
}
...
- (id) storedRangePreviewWindow
{
    ...
    if( isRGB)
        return [[[HorosPreviewWindow alloc] initWithLevel: 127.5f width: 255.f source: HorosPreviewWindowSourceColor] autorelease];
    
    // What the stored bits can hold, mapped through rescale. This is the range
    // the file could contain, not the range this frame happens to use, which is
    // why it is the last resort and not the first.
    int bits = bitsStored;
    if( bits < 1 || bits > 30)
        bits = (bitsAllocated > 0 && bitsAllocated <= 30) ? bitsAllocated : 16;
    
    float storedMinimum, storedMaximum;
    if( fIsSigned)
    {
        float magnitude = (float) (1L << (bits - 1));
        storedMinimum = -magnitude;
        storedMaximum = magnitude - 1.f;
    }
    else
    {
        storedMinimum = 0.f;
        storedMaximum = (float) ((1L << bits) - 1);
    }
    
    float first = storedMinimum * slope + offset;
    float second = storedMaximum * slope + offset;
    float low = first < second ? first : second;
    float high = first < second ? second : first;
    float range = high - low;
    if( range < 1.f) range = 1.f;
    
    if( isnan( range) || isinf( range) || isnan( low) || isinf( low))
        return nil;
    
    return [[[HorosPreviewWindow alloc] initWithLevel: low + range / 2.f width: range source: HorosPreviewWindowSourceStoredRange] autorelease];
#else
    return nil;
#endif
}
...
{
#ifndef DECOMPRESS_APP
    NSString *key = self.srcFile ? [HorosFileRevision cacheKeyForPath: self.srcFile] : nil;
    if( key)
        return key;
#endif
    return self.srcFile;
...
{
    return loadedFileRevision;
}

- (BOOL) loadedFileMatchesDisk
{
    if( loadedFileRevision == nil)
        return fImage == nil;	// nothing decoded, nothing stale; decoded without a record: do not trust it
#ifndef DECOMPRESS_APP
    return [(HorosFileRevision*) loadedFileRevision matchesDisk];
#else
    return YES;
#endif
}
    ...
    switch( stackMode)
    {
        ...
        case 5:		// Volume Rendering
            ...
            {											
                ...
                [thickSlab setWLWW: iwl: self.displayInverted ? -iww : iww];
...
- (BOOL) displayInverted { return inverseVal && !isRGB; }
...
{
    [self CheckLoad];
    return self.displayInverted ? -level : level;
}
...
- (void) compute8bitRepresentation
{
    ...
    if( baseAddr)
    {
        ...
        if( isRGB)
        {
            ...
            // A colour frame carries no scalar intensity to window, so a caller
            // that hands one a zero-width window is asking for the frame as it
            // is - not for a division by zero, which is what this used to be.
            if( diff == 0) diff = 1;
        ...
        // Polarity follows the VOI/transfer function. fImage remains in the
        // calibrated domain for measurements, session consumers and exports.
        if( self.displayInverted && !thickSlabVRActivated)
            for( NSUInteger i = 0; i < (NSUInteger) width * height; ++i)
                ((unsigned char*) baseAddr)[i] = 255 - ((unsigned char*) baseAddr)[i];
