...
//    display an error !
    ...
    switch( algorithm)
    {
        // Iso Contour: retain every rasterized voxel, including the image border.
        // The shared SEG extractor caps the mask at half-voxel boundaries.
        case 0:
        {
            NSData *vD = nil;
            NSMutableArray *copyPixList = nil;
            [vc copyVolumeData: &vD andDCMPix: &copyPixList forMovieIndex: vc.curMovieIndex];
            DCMPix *first = copyPixList.firstObject;
            NSMutableArray *frames = [NSMutableArray array];
            NSMutableArray *offsets = [NSMutableArray array];
            float orientation[9];
            [first orientation: orientation];
            for (NSUInteger z = 0; z < copyPixList.count; ++z)
            {
                DCMPix *p = [copyPixList objectAtIndex: z];
                memset(p.fImage, 0, p.pheight * p.pwidth * sizeof(float));
                for (ROI *curROI in [vc.roiList objectAtIndex: z])
                {
                    if ([[curROI name] isEqualToString: [roi name]])
                        [p fillROI:curROI newVal:1000 minValue:-FLT_MAX maxValue:FLT_MAX outside:NO orientationStack:2 stackNo:0 restore:NO addition:NO spline:[curROI isSpline] clipMin:NSMakePoint(0, 0) clipMax:NSMakePoint(p.pwidth, p.pheight)];
                }
                NSMutableData *mask = [NSMutableData dataWithLength:p.pheight * p.pwidth];
                unsigned char *bytes = (unsigned char *)mask.mutableBytes;
                for (NSUInteger index = 0; index < mask.length; ++index)
                    bytes[index] = p.fImage[index] > 0;
                [frames addObject:mask];
                double distance = (p.originX - first.originX) * orientation[6]
                    + (p.originY - first.originY) * orientation[7]
                    + (p.originZ - first.originZ) * orientation[8];
                [offsets addObject:@(factor * distance)];
            }
            HorosSEGSurfaceMesh *surface = [HorosSEGSurface meshForBinaryFrames:frames
