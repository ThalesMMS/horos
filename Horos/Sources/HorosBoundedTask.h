...
// behind.
static inline BOOL HorosRunTaskUntilExitCheckingCancellation(NSTask *task, NSTimeInterval timeout,
                                                              BOOL (^cancelled)(void), NSError **error)
{
    ...
    while (task.isRunning && NSProcessInfo.processInfo.systemUptime < deadline) {
        if (cancelled && cancelled()) { wasCancelled = YES; break; }
    ...
    if (task.isRunning) {
        ...
        if (error)
            *error = [NSError errorWithDomain:@"HorosTaskLaunch" code:3 userInfo:@{
                NSLocalizedDescriptionKey: wasCancelled ? @"The print job was cancelled." : @"The helper tool did not finish in time."}];
        return NO;
    }
...
{
    return HorosRunTaskUntilExitCheckingCancellation(task, timeout, nil, error);
}
