...
- (BOOL)createNewWordReportForStudy:(NSManagedObject*)study toDestinationPath:(NSString*)destinationFile
{
    ...
    NSError *consentError = [HorosWordReportAutomation consentErrorWithoutPrompt];
    if (consentError) {
        NSLog(@"***** Word report Automation unavailable: %@", consentError);
        NSRunCriticalAlertPanel(NSLocalizedString(@"Microsoft Word", nil), @"%@",
            NSLocalizedString(@"OK", nil), nil, nil, consentError.localizedDescription);
        return NO;
    }
    ...
    BOOL created = HorosCreateReportFromTemplate(templatePath, destinationFile, ^BOOL(NSString *prepared, NSError **error) {
        ...
        NSString *privateTemplate = [directory stringByAppendingPathComponent:
            [[directory.lastPathComponent stringByAppendingString:@"-template"] stringByAppendingPathExtension:prepared.pathExtension]];
        if (![NSFileManager.defaultManager moveItemAtPath:prepared toPath:privateTemplate error:error]) return NO;
        ...
        NSString *output = [directory stringByAppendingPathComponent:
            [[directory.lastPathComponent stringByAppendingString:@"-merged"] stringByAppendingPathExtension:extension]];
        if (![NSFileManager.defaultManager copyItemAtPath:privateTemplate toPath:output error:error]) return NO;
        @try {
            ...
            id result = [[self class] _runAppleScript:source withArguments:@[sourceData, output, privateTemplate, extension]];
