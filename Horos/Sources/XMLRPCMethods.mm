...
-(void)_onMainThreadOpenObjectsWithIDs:(NSArray*)objectIDs { // actually, only the first element is opened...
    // XML-RPC already acknowledges an asynchronous request, not a displayed
    // viewer. Keep that contract, but wait for the operator's modal dialog
    // before closing viewers or bringing a new one in front of it. The normal
    // run-loop mode avoids polling inside the modal loop; recheck on delivery
    // because a nested loop or another dialog may still be active.
    if (NSApp.modalWindow) {
        [self performSelector:_cmd withObject:objectIDs afterDelay:0.1 inModes:@[NSDefaultRunLoopMode]];
        return;
    }
