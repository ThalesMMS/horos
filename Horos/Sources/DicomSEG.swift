...
public enum DicomSEGDiagnosis: String, Equatable, Hashable, Error {
    ...
    case invalidSourceReference = "invalid-source-reference"
    case invalidColor = "invalid-display-color"
}
...
/// A source image reference from a SEG functional group. DICOM frame numbers
/// are one-based; an empty list references the complete SOP instance.
public struct DicomSEGSourceReference: Equatable {
    public var sopInstanceUID: String
...
public struct DicomSEGSegment: Equatable {
    ...
    /// Display color in sRGB. The codec converts to/from DICOM PCS CIELab.
    ...
    public var frameReferences: [[DicomSEGSourceReference]] = []
    /// Preserve imported PCS, including colors outside the display gamut, until edited.
    public var originalDisplayColorPCS: [UInt16]? = nil
    ...
    public static func == (lhs: DicomSEGSegment, rhs: DicomSEGSegment) -> Bool {
        lhs.number == rhs.number
            ...
            && lhs.frameReferences == rhs.frameReferences
            && lhs.originalDisplayColorPCS == rhs.originalDisplayColorPCS
    }
...
public enum DicomSEGCommand: Equatable {
    ...
    case setOpacity(number: UInt16, from: Double, to: Double)
    ...
    public static func == (lhs: DicomSEGCommand, rhs: DicomSEGCommand) -> Bool {
        ...
        case let (.setVisibility(n1, a1, b1), .setVisibility(n2, a2, b2)):
            return n1 == n2 && a1 == a2 && b1 == b2
        case let (.setOpacity(n1, a1, b1), .setOpacity(n2, a2, b2)):
            return n1 == n2 && a1 == a2 && b1 == b2
...
public final class DicomSEGStore: NSObject {
    ...
    private var surfaceOpacities: [String: Double] = [:]

    public init(document: DicomSEGDocument, surfaceOpacities: [String: Double] = [:]) {
        self.document = document
        self.surfaceOpacities = surfaceOpacities.filter { $0.value.isFinite && (0...1).contains($0.value) }
        super.init()
    }
    ...
    public func opacity(segment number: UInt16) -> Double {
        guard let index = index(of: number) else { return 1 }
        return surfaceOpacities[document.segments[index].trackingUID] ?? 1
    }

    @discardableResult
    public func setOpacity(_ opacity: Double, segment number: UInt16) -> Bool {
        guard index(of: number) != nil, opacity.isFinite else { return false }
        apply(.setOpacity(number: number, from: self.opacity(segment: number),
                          to: max(0, min(1, opacity))), record: true)
        return true
    }
    ...
    private func apply(_ command: DicomSEGCommand, record: Bool) {
        ...
        case let .setOpacity(number, _, to):
            if let i = index(of: number) { surfaceOpacities[document.segments[i].trackingUID] = to }
        case let .setMask(number, _, to, _):
            if let i = index(of: number) { document.segments[i].frames = to }
        case let .duplicate(source, created):
            surfaceOpacities[created.trackingUID] = opacity(segment: source)
    ...
    private func invert(_ command: DicomSEGCommand) {
        ...
        case let .setOpacity(number, from, _):
            if let i = index(of: number) { surfaceOpacities[document.segments[i].trackingUID] = from }
...
public enum DicomSEGCodec {
    ...
    public static func decode(_ data: Data) -> DicomSEGDocument {
        ...
        let metaLength = DicomBinary.data(elements, 0x0002, 0x0000)
        // Earlier local exports embedded the whole meta header in this UL.
        // Refuse that invalid format instead of reinterpreting its old bit order.
        if !metaLength.isEmpty && metaLength.count != 4 {
            return emptyDocument(diagnoses: [.notDICOM])
        }
        ...
        var frameReferences: [[DicomSEGSourceReference]] = []
        for item in perFrame {
            ...
            let sources = DicomBinary.sequence(item, 0x0008, 0x9124).flatMap {
                DicomBinary.sequence($0, 0x0008, 0x2112)
            }
            let references = sources.map { source -> DicomSEGSourceReference in
                let uid = DicomBinary.string(source, 0x0008, 0x1155)
                let text = DicomBinary.string(source, 0x0008, 0x1160)
                let values = text.isEmpty ? [] : text.split(separator: "\\", omittingEmptySubsequences: false)
                let numbers = values.compactMap { Int($0.trimmingCharacters(in: .whitespaces)) }
                if uid.isEmpty || numbers.count != values.count || numbers.contains(where: { $0 < 1 }) {
                    diagnoses.append(.invalidSourceReference)
                }
                return DicomSEGSourceReference(sopInstanceUID: uid,
                    sopClassUID: DicomBinary.string(source, 0x0008, 0x1150), frameNumbers: numbers)
            }
            frameReferences.append(references)
            frameSOPs.append(references.first?.sopInstanceUID ?? "")
        }
        ...
        for item in DicomBinary.sequence(elements, 0x0062, 0x0002) {
            ...
            var rgb: (Double, Double, Double) = (1, 0, 0)
            if !color.isEmpty {
                if let converted = DicomSEGColor.rgb(color) { rgb = converted }
                else { diagnoses.append(.invalidColor) }
            }
            var planes = Array(repeating: emptyPlane, count: spatialOrigins.count)
            var refs = Array(repeating: "", count: spatialOrigins.count)
            var sources = Array(repeating: [DicomSEGSourceReference](), count: spatialOrigins.count)
            for (index, assigned) in frameSegment.enumerated() where assigned == number {
                ...
                if index < frameSOPs.count {
                    ...
                    sources[spatial] = frameReferences[index]
                }
            }
            ...
            segments.append(DicomSEGSegment(
                ...
                frameReferences: sources,
                originalDisplayColorPCS: color.isEmpty ? nil : color
            ))
        }
    ...
    public static func encode(_ document: DicomSEGDocument) throws -> Data {
        ...
        for segment in document.segments {
            for (index, plane) in segment.frames.enumerated() {
                ...
                let sources = index < segment.frameReferences.count ? segment.frameReferences[index]
                    : (sop.isEmpty ? [] : [DicomSEGSourceReference(sopInstanceUID: sop,
                        sopClassUID: "1.2.840.10008.5.1.4.1.1.2", frameNumbers: [])])
                frameItems.append(perFrameItem(origin: origin, segment: segment.number, sources: sources))
                if document.kind == .fractional { pixel.append(plane) }
            }
        }
        if document.kind == .binary {
            pixel = packBinary(document.segments.flatMap(\.frames),
                               rows: document.geometry.rows, columns: document.geometry.columns)
        }
        if pixel.count % 2 != 0 { pixel.append(0) }
        ...
        dataset.append(DicomBinary.sq(0x0062, 0x0002, try document.segments.map(segmentItem)))
        ...
        if !document.identity.sourceSOPInstanceUIDs.isEmpty {
            dataset.append(DicomBinary.sq(0x0008, 0x1115, [referencedSeries(document.identity,
                sources: document.segments.flatMap(\.frameReferences).flatMap { $0 })]))
        }
    ...
    private static func unpack(_ pixel: Data, rows: Int, columns: Int, frames: Int, bits: Int, kind: DicomSEGKind) -> (frames: [Data], truncated: Bool) {
        ...
        if kind == .binary && bits == 1 {
            ...
            for frame in 0..<frames {
                ...
                for i in 0..<plane {
                    let bit = frame * plane + i
                    if bit / 8 < pixel.count {
                        unpacked[i] = (pixel[bit / 8] >> (bit % 8)) & 1
                    } else { truncated = true }
                }
                out.append(unpacked)
            }
    ...
    /// DICOM binary frames form one LSB-first bit stream. Only the complete
    /// Pixel Data value is padded to an even byte length, never each frame.
    private static func packBinary(_ frames: [Data], rows: Int, columns: Int) -> Data {
        let count = rows * columns
        var packed = Data(repeating: 0, count: (count * frames.count + 7) / 8)
        for (frameIndex, frame) in frames.enumerated() {
            for i in 0..<min(count, frame.count) where frame[i] != 0 {
                let bit = frameIndex * count + i
    ...
    private static func segmentItem(_ segment: DicomSEGSegment) throws -> [DicomBinary.Element] {
        let color: [UInt16]
        if let original = segment.originalDisplayColorPCS, let rgb = DicomSEGColor.rgb(original),
           rgb.0 == segment.color.r, rgb.1 == segment.color.g, rgb.2 == segment.color.b {
            color = original
        } else { color = try DicomSEGColor.pcs(segment.color) }
        return [
            ...
            DicomBinary.usArray(0x0062, 0x000D, color),
    ...
    private static func perFrameItem(origin: [Double], segment: UInt16, sources: [DicomSEGSourceReference]) -> [DicomBinary.Element] {
        ...
        if !sources.isEmpty {
            items.append(DicomBinary.sq(0x0008, 0x9124, [[
                DicomBinary.sq(0x0008, 0x2112, sources.map { source in
                    var fields = [DicomBinary.ui(0x0008, 0x1150, source.sopClassUID),
                                  DicomBinary.ui(0x0008, 0x1155, source.sopInstanceUID)]
                    if !source.frameNumbers.isEmpty {
                        fields.append(DicomBinary.is(0x0008, 0x1160,
                            source.frameNumbers.map(String.init).joined(separator: "\\")))
                    }
                    return fields
                })
            ]]))
    ...
    private static func referencedSeries(_ identity: DicomSEGIdentity, sources: [DicomSEGSourceReference]) -> [DicomBinary.Element] {
        let instances = identity.sourceSOPInstanceUIDs.map { uid -> [DicomBinary.Element] in
            [DicomBinary.ui(0x0008, 0x1150, sources.first(where: { $0.sopInstanceUID == uid })?.sopClassUID
...
/// PS3.3 C.10.7.1.1 uses ICC PCS Lab (D50), not an unsigned RGB triplet.
/// CoreGraphics/ColorSync performs the profile conversion, including D50/D65
/// adaptation. Keep only DICOM's 16-bit packing rule here.
private enum DicomSEGColor {
    private static let lab = CGColorSpace(name: CGColorSpace.genericLab)
    private static let srgb = CGColorSpace(name: CGColorSpace.sRGB)

    static func rgb(_ pcs: [UInt16]) -> (Double, Double, Double)? {
        guard pcs.count == 3, let lab, let srgb,
              let color = CGColor(colorSpace: lab, components: [
                CGFloat(pcs[0]) * 100 / 65535,
                CGFloat(pcs[1]) * 255 / 65535 - 128,
                CGFloat(pcs[2]) * 255 / 65535 - 128, 1]),
              let values = color.converted(to: srgb, intent: .relativeColorimetric, options: nil)?.components,
              values.count == 4, values.allSatisfy(\.isFinite) else { return nil }
        // PCS colors outside sRGB's gamut are clipped only for display.
        let rgb = values.prefix(3).map { Double(max(0, min(1, $0))) }
        return (rgb[0], rgb[1], rgb[2])
    }

    static func pcs(_ rgb: (r: Double, g: Double, b: Double)) throws -> [UInt16] {
        let components = [rgb.r, rgb.g, rgb.b]
        guard components.allSatisfy(\.isFinite), let srgb, let lab,
              let color = CGColor(colorSpace: srgb,
                components: components.map { CGFloat(max(0, min(1, $0))) } + [1]),
              let values = color.converted(to: lab, intent: .relativeColorimetric, options: nil)?.components,
              values.count == 4, values.allSatisfy(\.isFinite) else { throw DicomSEGDiagnosis.invalidColor }
        return [values[0] * 65535 / 100, (values[1] + 128) * 65535 / 255,
                (values[2] + 128) * 65535 / 255].map { UInt16(max(0, min(65535, $0)).rounded()) }
    }
}
...
enum DicomBinary {
    ...
    static func file(classUID: String, instanceUID: String, dataset: [Element]) -> Data {
        ...
        length.append(contentsOf: u16(4))
        length.append(contentsOf: u32(UInt32(metaBody.count)))
