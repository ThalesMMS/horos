...
static bool HorosRenderMetalVolume(void *context, vtkHorosFixedPointVolumeRayCastMapper *mapper,
                                   vtkRenderer *renderer, vtkVolume *volume)
{
    return [(VRView *)context horosRenderMetalImageForMapper:mapper renderer:renderer volume:volume];
}
...
- (void) allocateCPUMapper
{
    ...
    volumeMapper->SetImageRenderer(HorosRenderMetalVolume, self);
    volumeMapper->Update();
...
- (void) instantiateEngine: (int) e
{
    @try
    {
        ...
        switch( e)
        {
            ...
            case 2:         // Metal image, with the same mapper for CPU fallback.
...
{
    vtkImageData *input = reader ? reader->GetOutput() : nullptr;
    if (!input || !volume || !isfinite(factor) || factor <= 0)
        return nil;
        ...
        else if( tool == tBonesRemoval)
        {
            ...
            if( [self get3DPixelUnder2DPositionX:_mouseLocStart.x Y:_mouseLocStart.y pixel:pix position:pos value:&value maxOpacity: BONEOPACITY minValue: minValue])
            {
                ...
                for ( int m = 0; m < [[controller viewer2D] maxMovieIndex] ; m++)
                {
                    ...
                    [controller horosVolumeMetalRelease];
...
{
    [controller horosVolumeMetalRelease];
    float previousLOD = LOD, previousLowResLODFactor = lowResLODFactor;
    WaitRendering	*www;
    ...
    @try
    {
        ...
        if (engine == 2)
        {
            lowResLODFactor = previousLowResLODFactor;
            [self setLOD:previousLOD];
        }
...
{
    if( isRGB || data8 == nil || firstObject == nil || controller == nil)
        return;
    
    if( [[controller viewer2D] maxMovieIndex] <= 1)
        return;
    
    long width = [firstObject pwidth];
    if( width <= 0 || (long) dst8.width * dst8.height < width + 2)
        return;
    
    float low = ([controller minimumValue] + OFFSET16) * valueFactor;
    float high = ([controller maximumValue] + OFFSET16) * valueFactor;
    
    unsigned short *volume16 = (unsigned short*) data8 + width;
    volume16[ 0] = (unsigned short) (low < 0 ? 0 : (low > 65535 ? 65535 : low));
    volume16[ 1] = (unsigned short) (high < 0 ? 0 : (high > 65535 ? 65535 : high));
}
...
- (BOOL) recomputeValueFactorAfterRangeChange
{
    ...
    float previousFactor = valueFactor, previousOffset = OFFSET16;
    [self computeValueFactor];
    
    if( previousFactor == valueFactor && previousOffset == OFFSET16)
    {
        // Same mapping, wider range: only the guard voxels change.
        [self applyMovieRangeGuardTo16BitVolume];
        if( reader && data8)
        {
            reader->GetOutput()->Modified();
    ...
    [self movieChangeSource: data showWait: NO];
    [self setWLWW: wl :ww];
...
-(short) setPixSource:(NSMutableArray*)pix :(float*) volumeData
{
    ...
    else
    {
        ...
        // The 4D min/max guard is applied to the 16-bit copy after conversion
        // (applyMovieRangeGuardTo16BitVolume), not to the shared float volume.
        [self computeValueFactor];
        //		vImageConvert_FTo16U( &srcf, &dst8, -OFFSET16, 1./valueFactor, 0);
        [BrowserController multiThreadedImageConvert: @"FTo16U" :&srcf :&dst8 :-OFFSET16 :1./valueFactor];
        [self applyMovieRangeGuardTo16BitVolume];
...
- (Camera*) cameraWithThumbnail:(BOOL) produceThumbnail
{
    ...
    if( croppingBox)
    {
        ...
        // setCamera: applies imported/flythrough planes to the mapper while
        // hiding the editing widget. Capture the rendered crop, not that stale box.
        vtkPlaneCollection *appliedPlanes = volume && volume->GetMapper()
            ? volume->GetMapper()->GetClippingPlanes() : NULL;
        BOOL hasAppliedCrop = appliedPlanes && appliedPlanes->GetNumberOfItems() == 6;
        int planeCount = hasAppliedCrop ? 6 : planes->GetNumberOfPlanes();
        if( planeCount != 6)
            NSLog( @"****** camera crop plane count != 6");
        ...
        for( int i = 0; i < planeCount; i++)
        {
            ...
            vtkPlane *vtkPlane = hasAppliedCrop ? appliedPlanes->GetItem( i) : planes->GetPlane( i);
