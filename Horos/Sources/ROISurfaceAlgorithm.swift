...
public final class ROISurfaceAlgorithm: NSObject {
    ...
    public static func isClosed(_ mesh: ROISurfaceMesh) -> Bool {
        ...
        var edgeCounts: [DirectedEdge: Int] = [:]
        for (a, b, c) in mesh.faces {
            ...
            for pair in [(a, b), (b, c), (c, a)] {
                ...
                let undirected = DirectedEdge(from: min(pair.0, pair.1), to: max(pair.0, pair.1))
                edgeCounts[undirected, default: 0] += 1
                if edgeCounts[undirected, default: 0] > 2 { return false }
