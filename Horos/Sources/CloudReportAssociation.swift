...
private struct Reader {
    ...
    init(data: Data, offset: Int, explicit: Bool, littleEndian: Bool, limit: Int? = nil) {
        self.data = data
        self.limit = min(data.count, limit ?? data.count)
        self.offset = offset
        ...
        while offset + 8 <= limit {
            ...
            if tag == itemStart {
                ...
                if length == 0xFFFF_FFFF {
                    ...
                    let end = min(limit, offset + Int(length))
                    var nested = Reader(data: data, offset: offset, explicit: explicit, littleEndian: littleEndian, limit: end)
            ...
            if length == 0xFFFF_FFFF || vr == "SQ" || tag == 0x0008_1110 || tag == 0x0008_1115
                ...
                if length == 0xFFFF_FFFF {
                    ...
                    let end = min(limit, offset + length)
                    var nested = Reader(data: data, offset: offset, explicit: explicit, littleEndian: littleEndian, limit: end)
    ...
    private func peekTag() -> UInt32 {
        guard offset + 4 <= limit else { return 0 }
    ...
    private func readUInt16(at position: Int) -> UInt16 {
        guard position + 2 <= limit else { return 0 }
    ...
    private func readUInt32(at position: Int) -> UInt32 {
        guard position + 4 <= limit else { return 0 }
    ...
    private mutating func consume(_ length: Int) -> Data {
        guard length >= 0, offset <= limit else { return Data() }
        let end = min(limit, offset + length)
