#!/usr/bin/env python3
"""Time 3D MPR reconstructions in a running Horos, VTK alone and VTK plus Metal (#374).
...
Reports p50/p95 per state, the GPU reslice time the bridge recorded, the volume
bytes on the GPU and the process footprint before and after. The Metal path
runs after VTK has produced the plane, so its cost adds to VTK's: no gain is
presumed and none is claimed. See docs/mpr-metal-reslice-validation.md.
...
summary = {'label': args.label, 'iterations': args.iterations, 'plane': state['plane'], 'volume': state['volume']}
for key in ('vtk', 'metal'):
    ...
    print('%s %-5s p50 %.2f ms p95 %.2f ms (GPU reslice p50 %.3f ms) footprint %.1f MB volume %d bytes%s' % (
