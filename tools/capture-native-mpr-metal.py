...
parser.add_argument('--actual-size', action='store_true', help='apply the host 100-percent scale action to all planes')
parser.add_argument('--lod', type=float, help='fix image sample distance for a controlled comparison')
...
if args.actual_size:
    actions += ('for (id m374ScaleView in @[(id)[m374C mprView1], (id)[m374C mprView2], (id)[m374C mprView3]]) '
                '{ (void)[m374ScaleView actualSize:nil]; }\n')
...
m374S[@"voxelToWorld"] = (id)[(id)[(id)[m374C mprView1] vrView] mprVoxelToWorldTransform];
...
if (COPY_SCALARS) (void)[(NSData*)[(id)[(NSArray*)[(id)objc_getClass("ViewerController") get2DViewers] firstObject] volumeData] writeToFile:VOLUME atomically:YES];
...
expression = expression.replace('COPY_SCALARS', 'NO' if args.skip_volume else 'YES')
# Avoid optimized app locals and out-of-line libc++ math calls in LLDB.
renames = {name: 'm374Capture_' + name for name in ('d', 'p', 'r', 'w', 'h', 'wc', 'rois')}
renames['isfinite'] = '__builtin_isfinite'
expression = re.sub(r'"(?:\\.|[^"\\])*"|\b[A-Za-z_]\w*\b',
                    lambda match: renames.get(match[0], match[0]), expression)
