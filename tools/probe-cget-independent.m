...
__attribute__((constructor)) static void install(void) {
 ...
 [[NSNotificationCenter defaultCenter] addObserverForName:NSApplicationDidFinishLaunchingNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification*n){
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW,3*NSEC_PER_SEC),dispatch_get_main_queue(),^{
   ...
   for(NSDictionary *server in servers) {
    ...
    NSThread *worker=[[NSThread alloc] initWithBlock:^{@autoreleasepool{
     @try {
      ...
      [study setNoSmartMode:YES];[study setShowErrorMessage:YES];
   ...
   NSTimeInterval cancelAfter = [[[NSProcessInfo processInfo] environment][@"HOROS_CGET_CANCEL_AFTER_SECONDS"] doubleValue];
   if (!(cancelAfter > 0 && cancelAfter <= 20)) cancelAfter = 2;
   dispatch_after(dispatch_time(DISPATCH_TIME_NOW,(int64_t)(cancelAfter*NSEC_PER_SEC)),dispatch_get_main_queue(),^{
