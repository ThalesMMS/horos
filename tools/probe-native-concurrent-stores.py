...
from pydicom.encaps import generate_frames,encapsulate
...
if 'local-validation' not in run.parts or run.is_relative_to(root):parser.error('Use local-validation outside the checkout')
...
if options.jp2_source:
 for entry in manifest['instances']:
  if entry['syntax']!='1.2.840.10008.1.2.4.90':continue
  path=run/'input'/entry['file'];ds=pydicom.dcmread(path)
  ds.PixelData=encapsulate([imagecodecs.jpeg2k_encode(imagecodecs.jpeg2k_decode(frame),
      codecformat='JP2',bitspersample=12,level=0,reversible=True) for frame in generate_frames(ds.PixelData)])
  ds.save_as(path,enforce_file_format=True);entry['sha256']=hashlib.sha256(path.read_bytes()).hexdigest()
 manifest['diagnosticNonstandardJP2']=True
 (run/'input/manifest.json').write_text(json.dumps(manifest,indent=2)+'\n')
inputs=[str(run/'input'/e['file']) for e in manifest['instances']];(run/'invalid.dcm').write_bytes(b'Invalid synthetic DICOM for per-file diagnosis')
...
try:
 ...
 result=dict(case='native-concurrent-store',objectsPerPeer=count,framesPerPeer=frames,syntaxB=options.syntax_b,jp2SourceContainers=options.jp2_source,invalidInputsReported=2,pixelsChecked=True,sourcesUnchanged=True,overlap=True)
